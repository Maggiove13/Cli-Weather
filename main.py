...
def main():
    parser = argparse.ArgumentParser(
        description="Aplicación CLI para consultar el clima."
    )
    parser.add_argument("-location", help="Consultar el clima según locación")
    parser.add_argument(
        "-format",
        help="Elige el formato de salida: json, txt o csv",
    ...
    try:
        ...
        wheather = get_data(
            data
        )  # Obtenemos la respuesta pero filtrada, con solo las infos que queremos mostrar
        ...
        if output_format == "json":
            ...
            for (
                key,
                value,
            ) in wheather.items():  # Usar items() para iterar sobre clave-valor
                print(f"{key}: {value}")
